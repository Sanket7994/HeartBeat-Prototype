...
# Model for Selecting multiple different medical procedures for appointments and prescriptions
...
class ClientDataCollectionPool(models.Model):
    ...
    medical_procedure_history = models.CharField(max_length=250, blank=True, null=True)
    prescription_count = models.IntegerField(default=0, blank=True, null=True)
    medication_history = models.CharField(max_length=250, blank=True, null=True)
    transactions_made = models.CharField(max_length=100, blank=True, null=True)
    total_billings = models.IntegerField(default=0, blank=True, null=True)
    last_updated = models.DateTimeField(auto_now=True)
    created_at = models.DateTimeField(auto_now_add=True, editable=False)
...
class PatientAppointment(models.Model):
    ...
    patient_id = models.CharField(max_length=250, editable=False, unique=True, null=True, default=None)
    ...
    patient_age = models.IntegerField(default=0, editable=0)
    ...
    def save(self, *args, **kwargs):
        ...
        # Creating patient id for saving data in Client Database
        if not self.patient_id:
            while True:
                new_patient_id = str(uuid.uuid4().hex[:6].upper())
                if not ClientDataCollectionPool.objects.filter(client_id=new_patient_id).exists():
...
class PrescriptionManagement(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            appointment_data = PatientAppointment.objects.values('patient_id').get(id=data['appointment_id'])
            patient_id = appointment_data['patient_id']
            
            # Fetch client data
            client_data = ClientDataCollectionPool.objects.values().get(client_id=patient_id)
            medication_history = json.loads(data["medications"])
            
            print(medication_history)
            
            # Update client data
            client_data['prescription_count'] = get_count(patient_id, "prescription_count")
            client_data['medication_history'] = medication_history
            
            print(client_data['medication_history'])
            
            # Saving Client Data
            client_data_serializer = ClientDataCollectionPoolSerializer(
                instance=client_data, data=client_data, partial=True)
            client_data_serializer.is_valid(raise_exception=True)
            client_data_serializer.save()
            
            # Saving prescription Data
            ...
            raise NotFound("Client data not found")
...
class PrescribedMedicationModelHistory(APIView):
    ...
    def get(self, request, prescription_id, *args, **kwargs):
        try:
            queryset = PrescribedMedicationModel.objects.filter(prescription_id=prescription_id)
